...
var IntlMessageFormat = require('intl-formatmessage');
// Base action creator
...
module.exports = {
  ...
  T: function T(store) {
    return function (msgid, values, desc) {
      ...
      var msg = messages.get(msgid, fallbackMessage).msgid;
      var message = new IntlMessageFormat(msg, state.nabu.get('locale'));
      var text = message.format(values);
      if (markerOn) {
        text = '#' + text + '#';
      }
      return text;
